...
export const generateCss = (
  ...
  for (const layerName of LAYER_ORDER) {
    ...
    const layerContent = generateLayerContent(layerName, layerRules, extraction, {
      useScope,
      minify,
      indent,
    });
    ...
    if (baseClass === "_general") {
      ...
      const scopedContent = classRules
        .map((r) => {
          ...
          return r.replace(new RegExp(`^\\.${baseClass}\\b`), ":scope");
        })
...
export const generateScopeFallback = (
  ...
  for (const layerName of ["styled", "onevar", "resonevar", "allvar"] as const) {
    ...
    const fallbackRules = layerRules.map((rule) => {
      ...
      return rule.replace(/^(\.[\w-]+)/, (match, className) => `${className}:where(${className})`);
    });
    ...
    if (useLayers) {
      parts.push(
        `${indent}@layer ${layerPrefix}.${layerName} {${nl}${content}${nl}${indent}}${nl}`,
      );
    } else {
      parts.push(content);
    }
